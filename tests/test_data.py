from fedpss.data import generate_synthetic_bioarc, PatientDataset, attach_semantic_embeddings
...
def test_synthetic_records_have_expected_shapes():
    ...
    encoder = MockSemanticMapper(dim=64)
    attach_semantic_embeddings(records, encoder, batch_size=4)
    ...
    assert row["semantic_embedding"].shape == (64,)
    assert records[0].descriptors
