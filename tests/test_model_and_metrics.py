...
from fedpss.data import PatientDataset, attach_note_embeddings, collate_batch, generate_synthetic_bioarc
...
def test_model_forward_and_metrics():
    ...
    attach_note_embeddings(records, MockNoteEncoder(dim=64), batch_size=8)
    ds = PatientDataset(records, code_vocab_size=32, note_dim=64)
    loader = DataLoader(ds, batch_size=8, collate_fn=collate_batch)
    model = PatientSimilarityModel(static_dim=6, temporal_dim=5, code_vocab_size=32, note_dim=64, hidden_dim=32, embedding_dim=16)
